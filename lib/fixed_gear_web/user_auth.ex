defmodule FixedGearWeb.UserAuth do
  use FixedGearWeb, :verified_routes
  ...
  @remember_me_cookie "_fixed_gear_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      FixedGearWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      FixedGearWeb.Endpoint.broadcast(
  ...
  the `current_scope`:
      ...
      defmodule FixedGearWeb.PageLive do
        use FixedGearWeb, :live_view

        on_mount {FixedGearWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FixedGearWeb.UserAuth, :require_authenticated}] do
